...
export default function SalesList() {
  ...
  const loadSales = useCallback(async (t) => {
    ...
    try { const data = await ListSales(t); setSales(data || []) }
  ...
  const handleTabChange = (t) => { setFilter(t); loadSales(t) }

  const handleSavePrice = async () => {
    const buy = parseFloat(priceForm.buy), sell = parseFloat(priceForm.sell)
    if (!buy || !sell) return
    setSavingPrice(true)
    try { await UpsertTodayPrice(buy, sell); await loadPrice(); setEditPrice(false) }
    catch (e) { setError('บันทึกราคาไม่สำเร็จ: ' + e) }
    ...
    <div className="page-view">
      <div className="page-header">
        <div>
          ...
          <div className="page-meta">{!loading && `${sales.length} รายการ`}</div>
        </div>
        <div style={{ display:'flex', gap:8 }}>
          <button className="btn btn-ghost" onClick={() => setShowForm('buy')}><IconDown /> รับซื้อ</button>
          <button className="btn btn-primary" onClick={() => setShowForm('sell')}><IconUp /> ขายทอง</button>
        </div>
      </div>
      ...
      <div className="sl-price-bar card">
        <div className="sl-price-bar-inner">
          <div className="sl-price-label"><IconGold />ราคาทองวันนี้{price&&<span className="sl-price-date">{toBE(price.date)}</span>}</div>
          {!editPrice ? (
            <div className="sl-price-values">
              ...
              <button className="btn btn-ghost btn-sm" onClick={()=>setEditPrice(true)}><IconEdit/> แก้ไขราคา</button>
            </div>
          ) : (
            <div className="sl-price-edit">
              <div className="sl-price-edit-field"><span className="sl-price-type sell">ขาย</span><input className="input sl-price-input" type="number" value={priceForm.sell} onChange={e=>setPriceForm(p=>({...p,sell:e.target.value}))} autoFocus/></div>
              <div className="sl-price-edit-field"><span className="sl-price-type buy">รับซื้อ</span><input className="input sl-price-input" type="number" value={priceForm.buy} onChange={e=>setPriceForm(p=>({...p,buy:e.target.value}))}/></div>
              <button className="btn btn-primary btn-sm" onClick={handleSavePrice} disabled={savingPrice}>{savingPrice?'...':'บันทึก'}</button>
      ...
      <div className="toolbar">
        <div className="pl-tabs">
          {TYPE_TABS.map(t=>(
            <button key={t.value} className={`pl-tab ${filter===t.value?'pl-tab-active':''}`} onClick={()=>handleTabChange(t.value)}>{t.label}</button>
          ))}
        </div>
      </div>
      ...
      <div className="card">
        <div className="table-wrap">
          <table className="table">
            <thead>
              <tr>
                <th>วันที่</th><th>ประเภท</th><th>ลูกค้า</th><th>รายการทอง</th>
            ...
            <tbody>
              {loading ? (
                <tr className="loading-row"><td colSpan={9}>กำลังโหลด...</td></tr>
              ) : sales.length===0 ? (
                <tr className="loading-row"><td colSpan={9}>ยังไม่มีรายการ</td></tr>
              ) : sales.map(s=>(
                <tr key={s.id} style={{cursor:'default'}}>
                  <td className="sl-date">{toBE(s.date)}</td>
                  <td><span className={`badge ${s.type==='sell'?'badge-green':'badge-amber'}`}>{s.type==='sell'?'ขาย':'รับซื้อ'}</span></td>
                  <td className="sl-customer">{s.customer_name||<span style={{color:'var(--text-disabled)'}}>—</span>}</td>
                  ...
                  <td className="sl-num">{s.weight_baht?(s.total_amount/s.weight_baht).toLocaleString('th-TH'):'—'}</td>
                  ...
                  <td className="sl-print" onClick={()=>handlePrint(s)}>
                    <button className="btn btn-ghost btn-xs" title="พิมพ์ใบเสร็จ"><IconPrint/></button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {showForm && (
        <NewSaleForm defaultType={showForm} todayPrice={price}
          onSaved={()=>{setShowForm(null);loadSales(filter);loadPrice()}}
          onClose={()=>setShowForm(null)} />
      )}

      {printSale && shop && (
        <SaleReceipt sale={printSale} shop={shop} onClose={()=>setPrintSale(null)} />
      )}
    </div>
  )
}
...
function IconPrint() { return <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><polyline points="6 9 6 2 18 2 18 9"/><path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"/><rect x="6" y="14" width="12" height="8"/></svg> }
